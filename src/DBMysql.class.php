...
class DBMysql
{
    ...
    private static $tableDB = []; //当前初始化过的数据表操作实例
    private static $tableInfo = []; //缓存各表的字段信息 ['pk'] 主键字段名, ['ai'] 自增字段名, fields = [] 字段列表
    private static $log = []; //执行的SQL日志记录
...
//===========================
    ...
    //获取数据表的信息
    //参数: $table 表名
    //返回: $arr 缓存各表的字段信息 ['pk'] 主键字段名, ['ai'] 自增字段名, ['fields'] = [] 字段列表
    ...
    */
    function getr($sqlArr = [])
    {
        ...
        $sqlArr = is_array($sqlArr['values']) ? $sqlArr['values'] : [];
    ...
    */
    ...
    $size 每页数
    */
    function search($sqlArr = [], $page = 1, $size = 20)
    {
        if ($this->table == '') {
            return [];
        }
        !is_array($sqlArr) && $sqlArr = [];
    ...
    $size 每页数
    */
    function joinSearch($tables = [], $_sqlArr = [], $page = 1, $size = 20)
    {
        if (empty($tables)) {
            return [];
        }
        !is_array($_sqlArr) && $_sqlArr = [];
...
//===========================
    ...
    function insert($r = [], $returnId = false)
    {
    ...
    function insert_update($r = [], $setSqlArr = [])
    {
        !is_array($setSqlArr) && $setSqlArr = [];
...
//=================================
    ...
    {
        if (!is_array($sqlArr)) {
            return array('sql' => trim($sqlArr), 'data' => []);
        }

        $sql = array_shift($sqlArr);
        return array(
            'sql' => trim($sql),
            'data' => is_array($sqlArr) ? array_values($sqlArr) : []
        );
    }
