...
abstract class Action
{
    ...
    //=========================
    //取得外部传入的数据
    final protected function data(string $name = '', $default = NULL)
    {
    ...
    final protected function pageCahce(int $time = 0)
    {
        if (!CLN_IS_CLI) {
            $time = (int)$time;
            if ($time > 1) {
    ...
    final protected function IpPass(int $stime = 0, int $time = 0, int $maxNum = 0)
    {
        $stime = (int)$stime;
        $time = (int)$time;
        $maxNum = (int)$maxNum;
    ...
    //设置模板文件与数据
    final protected function display(string $tplFile = '', array $values = [])
    {
    ...
    final function __call(string $name, $arguments)
    {
