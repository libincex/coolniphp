...
class DataItem
{
	...
	//默认数据项与名称
	private static $dataitems = array(
		'int'=>array('type'=>'int'),
	...
	public static function reg($name,$type='text',$defualtValue=NULL)
	{
		...
		self::$dataitems[$name] = isset($defualtValue)?array('type'=>$type,'value'=>$defualtValue):array('type'=>$type);
	...
	*/
	public static function gets($formatList=array(),$_data=array())
	{
		!is_array($formatList) && $formatList = array();
		!is_array($_data) && $_data = array();
